...
export interface KubeConfigFileCluster {
  ...
  // status of import
  _status: string;
  // guid of the exising endpoint for this cluster
  _guid: string;
  // subtype
  _subType?: string;
  // register only - in the case that we don't support the auth mechanism, or need more information
  _registerOnly: boolean;
}
